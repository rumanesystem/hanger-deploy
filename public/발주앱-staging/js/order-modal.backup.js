...
function checkUpperColorCodes(color){
  ...
  tbody.querySelectorAll('tr[data-price-row]').forEach(tr=>{
    ...
    // 색상별 코드 확인 (샴페인골드↔골드 별칭 포함)
    const colorKey=resolveColorKey(color);
    const hasCd=!!(dbItem.colorProdCdMap&&(dbItem.colorProdCdMap[colorKey]||dbItem.colorProdCdMap[color]));
    inp.disabled=!hasCd;
    tr.style.opacity=hasCd?'':'.45';
...
function checkDrawerColorCodes(color){
  ...
  drawerBody.querySelectorAll('tr[data-price-row]').forEach(tr=>{
    ...
    // hasColor 품목(인출식 바지걸이 등): 개별 색상 선택 → 공통색 체크 제외
    if(dbItem.hasColor){
      inp.disabled=false;
      tr.style.opacity='';
      return;
    ...
    // noColor 품목 또는 prodCd만 있고 colorProdCdMap 없는 품목 → 색 무관, 항상 활성화
    if(dbItem.noColor||(dbItem.prodCd&&!dbItem.colorProdCdMap)){
      inp.disabled=false;
      tr.style.opacity='';
      return;
    }

    // colorProdCdMap 기준으로 해당 색상 코드 확인
    const map=dbItem.colorProdCdMap;
    const colorKey=resolveColorKey(color);
    const hasCd=!!(map&&(map[colorKey]||map[color]));
    inp.disabled=!hasCd;
    tr.style.opacity=hasCd?'':'.45';
    if(!hasCd) _insertNoCdBadge(tr,color+' 코드 없음');
  });
...
function _openOrderModalRender(initialData){
  ...
  const drawerItems=getItems().filter(i=>i.isActive && !(i.drawerType==='handle') && ['서랍장','옵션','서비스'].includes(i.category));
  const drawerBody=document.getElementById('drawer-body');
  drawerBody.innerHTML=drawerItems.map(item=>{
    ...
    // ── 세부 구성 행 (지정된 옵션 품목만: 거울장/디바이더/이불장/화장대세트) ──
    const SUBTYPE_ORDER_ITEMS=[]; // 임시 비활성화
    const subDefs=(typeof ITEM_SUBTYPES!=='undefined'&&SUBTYPE_ORDER_ITEMS.includes(item.name))?ITEM_SUBTYPES[item.name]:null;
    let subRows='';
    if(subDefs){
      subRows=subDefs.map(st=>{
        const uid=`subchk-${item.id}-${st.name.replace(/[^a-zA-Z0-9가-힣]/g,'_')}`;
        return `<tr class="subtype-row" data-parent-id="${item.id}" data-subtype-name="${st.name.replace(/"/g,'&quot;')}" style="display:none;background:#faf5ff">
          <td colspan="7" style="padding:5px 8px 5px 28px;border-left:3px solid #c4b5fd">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:12px;color:#5b21b6">
              <input type="checkbox" class="subtype-chk" id="${uid}"
                data-parent-id="${item.id}" data-subtype-name="${st.name.replace(/"/g,'&quot;')}"
                style="width:15px;height:15px;accent-color:#7c3aed;cursor:pointer">
              <span style="color:#c4b5fd;margin-right:2px">└</span>${st.name}
            </label>
          </td>
        </tr>`;
      }).join('');
  ...
  drawerBody.querySelectorAll('.drawer-qty').forEach(inp=>{
    inp.addEventListener('input',()=>{
      ...
      // ── 세부 구성 행 표시/숨김 ──
      const itemId=inp.dataset.itemId;
      drawerBody.querySelectorAll(`.subtype-row[data-parent-id="${itemId}"]`).forEach(tr=>{
...
function submitOrder(saveMode='발주확정'){
  ...
  if(saveMode!=='임시저장'){
    ...
    document.querySelectorAll('#upper-material-body .upper-qty').forEach(inp=>{
      ...
      const hasCd1=!!(map1&&(map1[colorKey1]||map1[ucColorForVal]));
    ...
    document.querySelectorAll('#drawer-body .drawer-qty').forEach(inp=>{
      ...
      const itemId=parseInt(inp.dataset.itemId);
      const dbItem=dbItemsForVal.find(i=>i.id===itemId);
      if(!dbItem||dbItem.hasColor||dbItem.noColor)return;
      // prodCd만 있고 colorProdCdMap 없는 색 무관 품목은 통과
      if(dbItem.prodCd&&!dbItem.colorProdCdMap)return;
      const map2=dbItem.colorProdCdMap;
      const colorKey2=resolveColorKey(scColorForVal);
      const hasCd2=!!(map2[colorKey2]||map2[scColorForVal]);
      if(!hasCd2) _markRequired(inp, dbItem.name+' — '+scColorForVal+' 색상 이카운트 코드가 없습니다. 품목 관리에서 코드를 입력해주세요.');
    });
  }
  ...
  document.querySelectorAll('.drawer-qty').forEach(inp=>{
    ...
    if(qty>0){
      ...
      // 세부 구성 체크 항목 수집
      const subTypeChecked=[];
      document.querySelectorAll(`.subtype-chk[data-parent-id="${itemId}"]`).forEach(chk=>{
        if(chk.checked)subTypeChecked.push(chk.dataset.subtypeName);
      });
      drawerItems.push({itemId,requiredQty:qty,color,handleOption:handleOpt,displayName,note:itemNote,unitPrice,amount:supply,vatAmount:vatAmt,subTypeChecked:subTypeChecked.length?subTypeChecked:undefined});
    }
  });
